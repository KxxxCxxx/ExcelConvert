    ...
    public partial class ExcelConvert : Form
    {
        ...
        private void fExcelSave()
        {
            ...
            try
            {
                ...
                worksheet.SaveAs(select_path + "\\" + "주문_" + DateTime.Now.ToString("yyyyMMddhhmmss"));
                    ...
                    for (int j = 1; j <= range.Columns.Count; ++j)
                    {
                        switch (j)
                        {
                            ...
                            case 23: //구매수량
                                ...
                                break;
                            case 13: //노출상품명 -> 영문주소
                                dataGridView1.Rows[idx].Cells[11].Value = (range.Cells[i, j] as Range).Value2.ToString();
                                break;
                            case 14: //노출상품ID -> 배송시요청사항
                                dataGridView1.Rows[idx].Cells[12].Value = (range.Cells[i, j] as Range).Value2.ToString();
                                break;
                            case 19: //결제액 -> 색상
                                dataGridView1.Rows[idx].Cells[15].Value = (range.Cells[i, j] as Range).Value2.ToString();
                                break;
                            default:
                                break;
                        }
